  ...
  &                 '|', atom_data%pos_cart(:, nat, nsp)*print_output%lenconfac, '|'
    ...
    if (pw90_calculation%berry .or. print_output%iprint > 2) then
      ...
      if (index(pw90_berry%task, 'floq') > 0) then
        ...
        write (stdout, '(1x,a46,10x,f8.3,13x,a1)') '|  Number points used to calculate TEV       :', pw90_berry%floq_ntstep, '|'
        write (stdout, '(1x,a46,10x,f8.3,13x,a1)') '|  Number harmonics to calculate FT-s        :', pw90_berry%floq_frange, '|'
        write (stdout, '(1x,a46,10x,f8.3,13x,a1)') '|  Lower time for Floquet calculation        :', pw90_berry%floq_time_min, '|'
        write (stdout, '(1x,a46,10x,f8.3,13x,a1)') '|  Upper time for Floquet calculation        :', pw90_berry%floq_time_max, '|'
        write (stdout, '(1x,a46,10x,f8.3,13x,a1)') '|  Time step for Floquet calculation         :', pw90_berry%floq_time_step, '|'
      endif
      if (index(pw90_berry%task, 'sc') > 0) then
